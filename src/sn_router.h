...
public:
    ...
    void  addRouting(const Address& dst, const Address& next_hop
		,int metric, const port_ptr& port)throw(std::exception)
	{

        auto i = std::find(_ports.begin(), _ports.end(), port);
        if(i != _ports.end() )
            _route_table.add(dst, next_hop, metric, port);
		else
